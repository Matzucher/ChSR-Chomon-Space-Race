...
public partial class AssembleRocket : Button
{
	...
	private new void ButtonPressed()
	{
		...
		foreach (RigidBody2D newBody in newBodies)
		{
			...
			Globals.Instance.GetNode<Node2D>(Globals.worldScenePath).AddChild(newBody);
            newBody.Scale = new Vector2(0.5, 0.5); //I have questions but I will not ask them ~jk
            newBody.Skew = 90; //I have even more questions
